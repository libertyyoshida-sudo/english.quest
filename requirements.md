...
「Language Quest」は、RPG 風のゲーム演出を通じて複数言語を楽しく学習できる Web アプリである。
本要件定義書は、現行の全機能を既存機能要件として整理し、さらに新機能「セーブ機能（進捗保存）」の要件を追加するものである。
...
- 英語以外の難易度表示ではTOEIC表記を使用せず、アプリ内レベル表記を使用する
- **Player**: アプリを使用する学習者
...
1. THE App SHALL フィールド画面に難易度選択（ぜんぶ / 下級 / 中級 / 上級）と問題数選択（5問 / 10問 / 20問）のセレクトボックスを表示する。
2. WHEN 難易度が「ぜんぶ」のとき、THE App SHALL 全難易度の問題プールから出題する。
3. WHEN 特定の難易度が選択されているとき、THE App SHALL 当該難易度の問題のみを出題する。
4. WHEN 出題可能な問題数が設定問題数に満たないとき、THE App SHALL アラートを表示してバトルを開始しない。
